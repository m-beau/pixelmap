...
Probe types are defined in PixelMap as Neuropixels probes sharing the same hardware characteristics (site geometry and wiring constraints).

Probe subtypes correspond to IMEC probe part numbers (e.g. NP1000, NP2013...). These numbers are defined by IMEC, you can find them on your IMEC order or probes box label. They uniquely correspond to numbers internally generated by spikeGLX: "probe type numbers". Probe types numbers are used by SpikeGLX to define the format of IMRO files. In SpikeGLX releases prior to 20260115, the this number was included in the first entry of IMRO files to identify probe subtypes. Release 20260115 and later switched to IMEC's part number - see [SpikeGLX's documentation on IMRO tables anatomy](https://billkarsh.github.io/SpikeGLX/help/imroTables/#terminology). If you need to use a version of SpikeGLX anterior to release 20260115, you can tick the "Legacy IMRO format" checkbox in PixelMap to generate IMRO files that will include SpikeGLX probe type number in their header.

:::{important}
If the **probe subtype** is set incorrectly, SpikeGLX will silently ignore your `.imro` files.

If this happens and you cannot find your probe's subtype, just plug it into SpikeGLX to generate either:
