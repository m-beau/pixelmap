...
```{=latex}
\makeatletter
% JOSS's template forces every figure to [H] (float package), which wraps the
% figure in a single unbreakable box. Figure 1's caption is long, so that box
% does not fit in the space left on the page and LaTeX pushes the whole figure
% to the next page, leaving a large gap. Contribute the figure to the main
% vertical list instead, and typeset the caption as an ordinary paragraph, so
% the caption can break across pages. PDF only: the markdown is untouched, so
% the JATS/HTML outputs are unaffected.
...
\long\def\@makecaption#1#2{%
  ...
  \begingroup\captionfont\noindent{\captionlabelfont #1:} #2\par\endgroup
