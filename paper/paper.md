...
Beyond satisfying hardware constraints, channelmaps must be tailored to the experiment. Before implantation, researchers plan which brain regions each probe will traverse given its intended insertion trajectory; this requires mapping anatomical boundaries onto the physical electrode layout. After implantation (chronic or acute), researchers must adjust their channelmap based on observed neural activity. Regions with large waveforms typically correspond to grey matter (rich in axon initial segments) and allow to identify target regions entry and exit points. The typical workflow is therefore: plan anatomically before implantation, survey activity afterwards, and refine the channelmap to maximise unit yield in the target regions. Using ultiple probes simultaneously, where the difficulty compounds, is becoming commonplace [@bondy2024], making a fast and reliable wiring-aware design tool increasingly essential. Developing a tool to support this workflow within the Brody laboratory motivated the creation of PixelMap, which addresses these needs by:

1. **Being available on any machine installation-free** at [https://pixelmap.pni.princeton.edu](https://pixelmap.pni.princeton.edu).
2. **Visualising in real time Neuropixels wiring constraints, interactively**: When users select electrodes (red), the interface immediately shows which other electrodes become unavailable (black) due to shared ADC lines, preventing invalid configurations.
...
- The full probe-dependent `.imro` metadata is exposed, including the Neuropixels 1.0 hardware filter and gain settings and the electrode reference (external, tip, or join-tips).
- A wider set of arbitrary electrode geometrie can be selected. In addition to a large set of custom presets, text entry of electrode ranges, and an `.imro` loader, it provides single-click and five drag-box selection/deselection tools, among them interleaved and checkerboard selectors for broader coverage, and a "deselect dependents" box, not available in existing tools, that identifies and releases the selected electrodes that are blocking a target region of unavailable electrodes.
