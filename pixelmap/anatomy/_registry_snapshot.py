...
Populates the GUI's atlas dropdown when the on-disk registry cache is missing
and the network is unreachable, so a cold cache degrades to a slightly stale
list instead of a blocking HTTP call on the server's event loop. See
:func:`pixelmap.anatomy.atlas.list_atlases`.

Generated, do not hand-edit. Refresh with ``python
scripts/refresh_registry_snapshot.py``.

Snapshot taken 2026-09-10 from brainglobe-atlasapi 3.0.1.
"""
